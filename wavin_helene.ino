...
void appendChannelCard(String &html, const ChannelData &data) {
  ...
  html += "<div class='card' id='card-" + channelId + "' data-ch='" + channelId + "'><div class='row'>";
...
void handleRoot() {
  ...
  html += "var SETPOINT_STEP_TENTHS = " + String(SETPOINT_STEP_TENTHS) + ";";
  html += "var pendingSetpoints = {};";
  html += "var pendingSetpointTimers = {};";
  html += "var pendingModes = {};";
  html += "var channelMissCounts = {};";
  html += "var CHANNEL_MISS_LIMIT = 3;";
  ...
  html += "function escapeHtml(value) { return String(value).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\"/g, '&quot;').replace(/'/g, '&#39;'); }";
  html += "function getChannelCard(ch) { return document.getElementById('card-' + ch); }";
  html += "function setChannelCardVisible(ch, visible) { var card = getChannelCard(ch); if (!card) return; card.style.display = visible ? '' : 'none'; }";
  html += "function insertChannelCard(item) { var ch = item.ch; var targetTenths = parseTempTenths(item.target); var container = document.getElementById('channels'); if (!container) return null; var existing = getChannelCard(ch); if (existing) { setChannelCardVisible(ch, true); return existing; } var div = document.createElement('div'); div.className = 'card'; div.id = 'card-' + ch; div.setAttribute('data-ch', String(ch)); div.innerHTML = \"<div class='row'><div class='col-left'><b id='name-\" + ch + \"'>\" + escapeHtml(item.name) + \"</b><br><span id='temp-\" + ch + \"'>\" + Number(item.temp).toFixed(1) + \"\\u00b0C</span><div class='badges'><span class='badge \" + (item.heating ? 'badge-heat' : 'badge-off') + \"' id='heat-\" + ch + \"'>\" + (item.heating ? '\\ud83d\\udd25 Varme' : 'Sluk') + \"</span><span class='badge badge-mode' id='mode-\" + ch + \"' onclick='toggleMode(\" + ch + \")' style='cursor:pointer'>\" + (item.mode === 0 ? 'Manuel' : 'Standby') + \"</span></div></div><div class='col-right'><div class='battery'>Batteri: <span id='battery-\" + ch + \"'>\" + item.battery + \"%</span></div><div class='controls'><button class='btn' onclick='adjust(\" + ch + \",-\" + SETPOINT_STEP_TENTHS + \")'>-</button><span class='target' id='target-\" + ch + \"' data-target-tenths='\" + targetTenths + \"'>\" + Number(item.target).toFixed(1) + \"\\u00b0C</span><button class='btn' onclick='adjust(\" + ch + \",\" + SETPOINT_STEP_TENTHS + \")'>+</button></div><div class='standby'>Standby: <span id='standby-\" + ch + \"'>\" + Number(item.standby).toFixed(1) + \"\\u00b0C</span></div></div></div>\"; for (var i = 0; i < container.children.length; i++) { var siblingCh = parseInt(container.children[i].getAttribute('data-ch'), 10); if (!isNaN(siblingCh) && siblingCh > ch) { container.insertBefore(div, container.children[i]); return div; } } container.appendChild(div); return div; }";
  html += "function updateChannelCard(item) { insertChannelCard(item); setChannelCardVisible(item.ch, true); channelMissCounts[item.ch] = 0; var temp = document.getElementById('temp-' + item.ch); if (temp) temp.textContent = formatTemp(item.temp); var name = document.getElementById('name-' + item.ch); if (name) name.textContent = item.name; var targetTenths = parseTempTenths(item.target); if (Object.prototype.hasOwnProperty.call(pendingSetpoints, item.ch)) { if (targetTenths === pendingSetpoints[item.ch] && !pendingSetpointTimers[item.ch]) { delete pendingSetpoints[item.ch]; } else { targetTenths = pendingSetpoints[item.ch]; } } else { setTargetValue(item.ch, targetTenths); } var standby = document.getElementById('standby-' + item.ch); if (standby) standby.textContent = formatTemp(item.standby); var battery = document.getElementById('battery-' + item.ch); if (battery) battery.textContent = item.battery + '%'; updateBadge('heat-' + item.ch, item.heating ? '🔥 Varme' : 'Sluk', item.heating ? 'badge-heat' : 'badge-off'); var modeVal = item.mode; if (Object.prototype.hasOwnProperty.call(pendingModes, item.ch)) { if (modeVal === pendingModes[item.ch]) { delete pendingModes[item.ch]; } else { modeVal = pendingModes[item.ch]; } } updateBadge('mode-' + item.ch, modeVal === 0 ? 'Manuel' : 'Standby', 'badge-mode'); }";
  html += "function refreshData() { fetch('/data').then(function(response) { return response.json(); }).then(function(items) { var activeChs = {}; items.forEach(function(item) { activeChs[item.ch] = true; updateChannelCard(item); }); var container = document.getElementById('channels'); if (!container) return; for (var i = 0; i < container.children.length; i++) { var card = container.children[i]; var ch = card.getAttribute('data-ch'); if (!activeChs[ch]) { channelMissCounts[ch] = (channelMissCounts[ch] || 0) + 1; if (channelMissCounts[ch] >= CHANNEL_MISS_LIMIT) { setChannelCardVisible(ch, false); if (Object.prototype.hasOwnProperty.call(pendingSetpoints, ch)) { delete pendingSetpoints[ch]; if (pendingSetpointTimers[ch]) { clearTimeout(pendingSetpointTimers[ch]); delete pendingSetpointTimers[ch]; } } if (Object.prototype.hasOwnProperty.call(pendingModes, ch)) { delete pendingModes[ch]; } } } } }).catch(function() {}); }";
  ...
  html += "</script></head><body><h1>Wavin Styring</h1><div id='channels'>";
